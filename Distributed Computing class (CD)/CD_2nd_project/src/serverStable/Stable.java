...
public class Stable {
    ...
    public Stable(Logger_Stub log, SantaHouse_Stub house,InitialData data) {
        this.log = log;
        this.house = house;
        this.stableCont = 0;
        this.sledgeCont = 0;
    ...
    public void groupAtTheSledge(int id) {
        try {
            ...
            this.sledgeCont++;
            if (sledgeCont == data.getnReindeers()) {
                santa.release();
                sledgeCont = 0;
            }
            mutex.release();
    ...
    public void harnessTheReindeers() {
        try {
            ...
            mutex.release();
            for (int i = 0; i < data.getnReindeers(); i++) {               //Pai natal liberta as renas
